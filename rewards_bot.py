...
def auto_limiter_on_success() -> None:
    global auto_limiter_delay, auto_limiter_bucket_limit, auto_limiter_success_streak

    if not AUTO_LIMITER_ENABLED:
        return

    auto_limiter_success_streak += 1
    if auto_limiter_success_streak < AUTO_LIMITER_RECOVERY_SUCCESS_STREAK:
        return
...
def auto_limiter_on_429() -> None:
    global auto_limiter_delay, auto_limiter_bucket_limit, auto_limiter_last_429_at, auto_limiter_success_streak
    ...
    if auto_limiter_bucket_limit > 3:
        auto_limiter_bucket_limit -= 1
        if auto_limiter_bucket_limit < 3:
            auto_limiter_bucket_limit = 3
...
async def channel_edit_worker() -> None:
    while True:
        ...
        try:
            ...
            pending_state = pending_channel_edits.get(channel.id)

            if pending_state and pending_state.get("target_name") != new_name:
                new_name = pending_state["target_name"]
            ...
            pending_state = pending_channel_edits.get(channel.id)
            if pending_state and pending_state.get("future") is result_future:
                pending_channel_edits.pop(channel.id, None)
...
async def clear_pending_channel_updates() -> int:
    ...
    for pending in pending_channel_edits.values():
        future = pending.get("future")
        if future is not None and not future.done():
            future.set_result(False)
    pending_channel_edits.clear()
...
async def safe_edit_channel_name(channel: discord.abc.GuildChannel | None, new_name: str):
    ...
    async with lock:
        ...
        pending = pending_channel_edits.get(channel.id)
        if pending:
            pending["target_name"] = new_name
            logging.info(
                "[QUEUE] Zastąpiono oczekującą zmianę kanału %s nową nazwą: %s",
                channel.id,
                new_name,
            )
        ...
        pending_channel_edits[channel.id] = {
...
async def apply_channel_name_updates_startup_fast(
    ...
    *,
    ...
    """Pierwsza synchronizacja dalej jest szybka, ale przechodzi przez bezpieczny limiter."""
    changed = 0
    ...
    for key, new_name in payload.items():
        ...
        ok = await safe_edit_channel_name(channel, after)
        if ok:
            logging.info("%s [FAST START SAFE] %s -> %s (id=%s)", log_prefix, before, after, channel.id)
            changed += 1

    logging.info("%s [FAST START SAFE] Zaktualizowano %s kanałów dla serwera %s", log_prefix, changed, guild.name)
    return changed
